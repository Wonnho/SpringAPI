...
public class PostControllerV42 {
    private final PostServiceV42 postServiceV42;
...
//    }
        ...
        return ResponseEntity
                ...
                .body(
                        ApiResponse.ok(
                                "게시글이 정상적으로 작성되었습니다",
                                "CREATED",
                                postServiceV42.createPost(reqeustDto)
                        )
                );
    ...
    public ResponseEntity<ApiResponse<List<PostListResponseDto>>> readPosts(){
        List<PostListResponseDto> data = postServiceV42.readPosts();
    ...
    public ResponseEntity<ApiResponse<PostResponseDto>> readPostById(@PathVariable Long id){
        ...
        return ResponseEntity.ok(ApiResponse.ok(postServiceV42.readPostById(id)));
    }
    ...
    public ResponseEntity<ApiResponse<PostResponseDto>> updatePost(@PathVariable Long id, @RequestBody PostUpdateRequestDto requestDto){
        return ResponseEntity.ok(ApiResponse.ok(postServiceV42.updatePost(id, requestDto)));
    }
